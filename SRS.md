...
* **BR-002 (Skill Matching Hierarchy):**
  ...
  * **Semantic Match ($S_{\text{semantic}}$):** Vector embedding / TF-IDF Cosine Similarity evaluating equivalent technologies (e.g. *PostgreSQL* $\approx$ *SQL*, *PyTorch* $\approx$ *Deep Learning*).
...
### 4.3 Hybrid ATS Scoring Module
* **FR-SCR-001:** The system SHALL calculate $S_{\text{keyword}}$ by computing exact skill intersections against JD mandatory skills.
* **FR-SCR-002:** The system SHALL calculate $S_{\text{semantic}}$ by building TF-IDF n-gram vectors and computing Cosine Similarity between vector arrays.
